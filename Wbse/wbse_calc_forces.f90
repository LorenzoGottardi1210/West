...
MODULE wbse_forces
  ...
  PRIVATE
  !
  PUBLIC :: wbse_calc_forces
  PUBLIC :: wbse_calc_nacs
  !
  CHARACTER(LEN=*), PARAMETER :: FMT_FORCE = "(5X,'atom ',I4,' type ',I2,'   force = ',3F14.8)"
  CHARACTER(LEN=*), PARAMETER :: FMT_EENAC = "(5X,'atom ',I4,' type ',I2,'   eeNAC = ',3F14.8)"
  CHARACTER(LEN=*), PARAMETER :: FMT_GENAC = "(5X,'atom ',I4,' type ',I2,'   geNAC = ',3F14.8)"
  !
  CONTAINS
    !
    !-----------------------------------------------------------------------
    SUBROUTINE wbse_calc_forces(dvg_exc_tmp)
      ...
      ! drhox1
      !
      CALL wbse_calc_drhox1(dvg_exc_tmp,drhox1,.FALSE.)
      !
      CALL wbse_forces_nacs_drhox1(n,dvg_exc_tmp,drhox1,forces,.FALSE.)
      !
      ! < dvg | dvg >
      !
...
#endif
      !
      CALL wbse_calc_dvgdvg_mat(dvg_exc_tmp,dvgdvg_mat,.FALSE.)
      !
      ! drhox2
      !
      ...
      !
      CALL wbse_forces_nacs_drhox2(n,dvgdvg_mat,drhox2,forces,.FALSE.)
      !
      ! Z vector
      !
...
#endif
      !
      CALL wbse_forces_nacs_drhoz(n,zvector,forces,.FALSE.)
      !
      ...
      DO ia = 1,nat
         ...
         ! forces = - gradients
         !
         WRITE(stdout,FMT_FORCE) ia,ityp(ia),(-forces(3*ia-3+ipol),ipol = 1,3)
         !
      ENDDO
      !
      IF(mpime == root) THEN
    ...
    !
    !-----------------------------------------------------------------------
    SUBROUTINE wbse_calc_nacs(dvg_exc_tmp_I,dvg_exc_tmp_J,omega_JI)
      !-----------------------------------------------------------------------
      !
      USE io_global,            ONLY : stdout
      USE kinds,                ONLY : DP
      ...
      USE noncollin_module,     ONLY : npol
      USE fft_base,             ONLY : dffts
      USE westcom,              ONLY : logfile,nbndval0x,n_trunc_bands,evc1_all,l_genac,l_eenac,&
                                     & do_eenac
      USE distribution_center,  ONLY : kpt_pool,band_group
      USE json_module,          ONLY : json_file
      USE mp_world,             ONLY : mpime,root
      ...
      ELSEIF(l_eenac .AND. do_eenac) THEN
         ...
         !
         IF(.NOT. PRESENT(dvg_exc_tmp_J)) CALL errore('wbse_calc_nacs','eeNAC needs state J',1)
         IF(.NOT. PRESENT(omega_JI)) CALL errore('wbse_calc_nacs','eeNAC needs omega_JI',1)
         !
      ...
      IF(l_eenac .AND. do_eenac) THEN
         ...
         ! drhox1
         !
         CALL wbse_calc_drhox1(dvg_exc_tmp_I,drhox1,.TRUE.,dvg_exc_tmp_J)
         !
         CALL wbse_forces_nacs_drhox1(n,dvg_exc_tmp_I,drhox1,nac_vec,.TRUE.,dvg_exc_tmp_J,omega_JI)
         !
         ! < dvgI | dvgJ >
         !
         CALL mp_waitall(reqs)
         ...
         CALL wbse_calc_drhox2(dvgdvg_mat,drhox2)
         !
         CALL wbse_forces_nacs_drhox2(n,dvgdvg_mat,drhox2,nac_vec,.TRUE.,omega_JI)
         !
      ENDIF
      !
      ! Z vector
      ...
      !
      IF(l_eenac .AND. do_eenac) THEN
         CALL build_rhs_zvector_eq_eenac(dvg_exc_tmp_I,dvg_exc_tmp_J,dvgdvg_mat,dvgdvg_mat_JI,&
         & drhox1,drhox2,z_rhs_vec,omega_JI)
      ENDIF
      !
      CALL solve_zvector_eq_cg(z_rhs_vec,zvector)
      !
...
#endif
      !
      CALL wbse_forces_nacs_drhoz(n,zvector,nac_vec,.TRUE.)
      !
      !$acc exit data delete(z_rhs_vec,zvector)
      DEALLOCATE(z_rhs_vec)
      DEALLOCATE(zvector)
      ...
      IF(l_genac .AND. .NOT. do_eenac) THEN
         ...
         DO ia = 1,nat
            !
            WRITE(stdout,FMT_GENAC) ia,ityp(ia),(nac_vec(3*ia-3+ipol),ipol = 1,3)
            !
         ENDDO
         !
      ELSEIF(l_eenac .AND. do_eenac) THEN
         ...
         DO ia = 1,nat
            !
            WRITE(stdout,FMT_EENAC) ia,ityp(ia),(nac_vec(3*ia-3+ipol),ipol = 1,3)
            !
         ENDDO
         !
      ENDIF
    ...
    !
    !-----------------------------------------------------------------------
    SUBROUTINE wbse_calc_drhox1(dvg_exc_tmp,drhox1,l_nac,dvg_exc_tmp_J)
      !-----------------------------------------------------------------------
      !
      USE kinds,                ONLY : DP
      ...
      & CALL errore('wbse_calc_drhox1','eeNAC needs state J',1)
      !
      dffts_nnr = dffts%nnr
      drhox1(:,:) = (0._DP,0._DP)
      !
      ...
      DO iks = 1,kpt_pool%nloc
         ...
         DO lbnd = 1,nbnd_do-MOD(nbnd_do,2),2
            ...
            IF(l_nac) THEN
               ...
               & dvg_exc_tmp_J(:,lbnd+1,iks),psic_J,'Wave')
               !
    ...
    !
    !-----------------------------------------------------------------------
    SUBROUTINE wbse_forces_nacs_drhox1(n,dvg_exc_tmp,drhox1,outvec,l_nac,dvg_exc_tmp_J,omega_JI)
      !-----------------------------------------------------------------------
      !
      USE io_global,            ONLY : stdout
      USE kinds,                ONLY : DP
      ...
      INTEGER, INTENT(IN) :: n
      COMPLEX(DP), INTENT(IN) :: dvg_exc_tmp(npwx*npol,band_group%nlocx,kpt_pool%nloc)
      COMPLEX(DP), INTENT(IN) :: drhox1(dffts%nnr,nspin)
      REAL(DP), INTENT(INOUT) :: outvec(n)
      LOGICAL, INTENT(IN) :: l_nac
      COMPLEX(DP), INTENT(IN), OPTIONAL :: dvg_exc_tmp_J(npwx*npol,band_group%nlocx,kpt_pool%nloc)
      REAL(DP), INTENT(IN), OPTIONAL :: omega_JI
      !
      ...
      REAL(DP), ALLOCATABLE :: vec_drhox1(:),veclc(:,:),rdrhox1(:,:)
      TYPE(json_file) :: json
      INTEGER :: iunit
      TYPE(bar_type) :: barra
      INTEGER, PARAMETER :: flks(2) = [2,1]
...
#endif
      !
      ALLOCATE(vec_drhox1(n))
      ALLOCATE(veclc(3,nat))
            ...
            DO ipol = 1,3
               ...
               IF(l_nac) THEN
                  ...
                  DO lbnd = 1,nbnd_do
                     DO ig = 1,npw
                        reduce = reduce &
                        & + REAL(dvg_exc_tmp_J(ig,lbnd,iks),KIND=DP)*REAL(dvpsi(ig,lbnd,ipol),KIND=DP) &
               ...
               ELSE
                  ...
                  DO lbnd = 1,nbnd_do
                     DO ig = 1,npw
                        reduce = reduce &
                        & + REAL(dvg_exc_tmp(ig,lbnd,iks),KIND=DP)*REAL(dvpsi(ig,lbnd,ipol),KIND=DP) &
               ...
               IF(gstart == 2) THEN
                  IF(l_nac) THEN
                     ...
                     DO lbnd = 1,nbnd_do
                        reduce = reduce &
                        & - REAL(dvg_exc_tmp_J(1,lbnd,iks),KIND=DP)*REAL(dvpsi(1,lbnd,ipol),KIND=DP)
                     ENDDO
                     !$acc end parallel
                  ELSE
                     ...
                     DO lbnd = 1,nbnd_do
                        reduce = reduce &
                        & - REAL(dvg_exc_tmp(1,lbnd,iks),KIND=DP)*REAL(dvpsi(1,lbnd,ipol),KIND=DP)
                     ENDDO
                     !$acc end parallel
                  ENDIF
               ENDIF
               !
               vec_drhox1(3*ia-3+ipol) = vec_drhox1(3*ia-3+ipol) + this_wk*reduce
               !
            ENDDO
            !
      ...
      ENDDO
      !
      CALL mp_sum(vec_drhox1,intra_bgrp_comm)
      CALL mp_sum(vec_drhox1,inter_bgrp_comm)
      CALL mp_sum(vec_drhox1,inter_pool_comm)
      !
      ...
      ENDIF
      !
      CALL force_lc(nat,tau,ityp,ntyp,alat,omega,ngm,ngl,igtongl,g,rdrhox1(:,1),gstart,gamma_only,&
      & vloc,veclc)
      !
      veclc(:,:) = -factor*veclc
      !
      DO ia = 1,nat
         DO ipol = 1,3
            vec_drhox1(3*ia-3+ipol) = vec_drhox1(3*ia-3+ipol) + veclc(ipol,ia)
         ENDDO
      ENDDO
      !
      IF(l_nac) vec_drhox1(:) = vec_drhox1 / (-omega_JI)
      !
      outvec(:) = outvec+vec_drhox1
      !
      IF(l_nac) THEN
         !
         ...
         DO ia = 1,nat
            !
            WRITE(stdout,FMT_EENAC) ia,ityp(ia),(vec_drhox1(3*ia-3+ipol),ipol = 1,3)
            !
         ENDDO
         !
      ELSE
         ...
         DO ia = 1,nat
            ...
            ! forces = - gradients
            !
            WRITE(stdout,FMT_FORCE) ia,ityp(ia),(-vec_drhox1(3*ia-3+ipol),ipol = 1,3)
            !
         ENDDO
         !
      ENDIF
      ...
      IF(mpime == root) THEN
         ...
         CALL json%load(filename=TRIM(logfile))
         IF(l_nac) THEN
            CALL json%add('output.nac_vec.eenac_drhox1',vec_drhox1(1:n))
         ELSE
            CALL json%add('output.forces.forces_drhox1',-vec_drhox1(1:n))
         ENDIF
         !
         OPEN(NEWUNIT=iunit,FILE=TRIM(logfile))
         CALL json%print(iunit)
    ...
    !
    !-----------------------------------------------------------------------
    SUBROUTINE wbse_calc_dvgdvg_mat(dvg_exc_tmp,dvgdvg_mat,l_nac,dvg_exc_tmp_J)
      !-----------------------------------------------------------------------
      !
      USE kinds,                ONLY : DP
      ...
      !
      IF(l_nac .AND. (.NOT. PRESENT(dvg_exc_tmp_J))) &
      & CALL errore('wbse_calc_dvgdvg_mat','eeNAC needs state J',1)
      !
      ...
      DO iks = 1,kpt_pool%nloc
         ...
         ELSE
            CALL glbrak_gamma(evc1_all(:,n_trunc_bands+1:nbndval,iks),dvg_exc_tmp(:,:,iks),&
            & dvgdvg_mat(:,:,iks),npw,npwx,nbndval-n_trunc_bands,nbnd_do,nbndval0x-n_trunc_bands,npol)
         ENDIF
         !
    ...
    !
    !-----------------------------------------------------------------------
    SUBROUTINE wbse_forces_nacs_drhox2(n,dvgdvg_mat,drhox2,outvec,l_nac,omega_JI)
      !-----------------------------------------------------------------------
      !
      USE io_global,            ONLY : stdout
      USE kinds,                ONLY : DP
      ...
      REAL(DP) :: reduce,factor,this_wk
      REAL(DP), ALLOCATABLE :: vec_drhox2(:),rdrhox2(:,:),veclc(:,:)
      TYPE(json_file) :: json
      INTEGER :: iunit
      TYPE(bar_type) :: barra
      INTEGER, PARAMETER :: flks(2) = [2,1]
      !
      IF(l_nac) THEN
         IF(.NOT. PRESENT(omega_JI)) CALL errore('wbse_forces_nacs_drhox2','eeNAC needs omega_JI',1)
      ...
      DO iks = 1,kpt_pool%nloc
         ...
         DO ia = 1,nat
            ...
            !
            ! 2) vec_drhox2 = < evc_iv2 | dvpsi_ia_iv >
            !
            !$acc host_data use_device(evc,dvgdvg_mat,aux2)
            CALL DGEMM('N','N',2*npw,band_group%nloc,nbndval-n_trunc_bands,1._DP,&
            & evc(1,n_trunc_bands+1),2*npwx,dvgdvg_mat(1,1,iks),nbndval0x-n_trunc_bands,0._DP,aux2,&
            & 2*npwx)
            !$acc end host_data
            !
            DO ipol = 1,3
               ...
               ENDIF
               !
               vec_drhox2(3*ia-3+ipol) = vec_drhox2(3*ia-3+ipol) - this_wk*reduce
               !
            ENDDO
            !
      ...
      ENDDO
      !
      CALL mp_sum(vec_drhox2,intra_bgrp_comm)
      CALL mp_sum(vec_drhox2,inter_bgrp_comm)
      CALL mp_sum(vec_drhox2,inter_pool_comm)
      !
      ...
      ENDIF
      !
      CALL force_lc(nat,tau,ityp,ntyp,alat,omega,ngm,ngl,igtongl,g,rdrhox2(:,1),gstart,gamma_only,&
      & vloc,veclc)
      !
      veclc(:,:) = -factor*veclc
      !
      DO ia = 1,nat
         DO ipol = 1,3
            vec_drhox2(3*ia-3+ipol) = vec_drhox2(3*ia-3+ipol)+veclc(ipol,ia)
         ENDDO
      ENDDO
      !
      IF(l_nac) vec_drhox2(:) = vec_drhox2 / (-omega_JI)
      !
      outvec(:) = outvec+vec_drhox2
      !
      IF(l_nac) THEN
         !
         ...
         DO ia = 1,nat
            !
            WRITE(stdout,FMT_EENAC) ia,ityp(ia),(vec_drhox2(3*ia-3+ipol),ipol = 1,3)
            !
         ENDDO
         !
      ELSE
         ...
         DO ia = 1,nat
            ...
            ! forces = - gradients
            !
            WRITE(stdout,FMT_FORCE) ia,ityp(ia),(-vec_drhox2(3*ia-3+ipol),ipol = 1,3)
            !
         ENDDO
         !
      ENDIF
      ...
      IF(mpime == root) THEN
         ...
         CALL json%load(filename=TRIM(logfile))
         IF(l_nac) THEN
            CALL json%add('output.nac_vec.eenac_drhox2',vec_drhox2(1:n))
         ELSE
            CALL json%add('output.forces.forces_drhox2',-vec_drhox2(1:n))
         ENDIF
         !
         OPEN(NEWUNIT=iunit,FILE=TRIM(logfile))
         CALL json%print(iunit)
    ...
    !
    !-----------------------------------------------------------------------
    SUBROUTINE wbse_forces_nacs_drhoz(n,zvector,outvec,l_nac)
      !-----------------------------------------------------------------------
      !
      USE io_global,            ONLY : stdout
      USE kinds,                ONLY : DP
      ...
      INTEGER :: band_group_myoffset
      REAL(DP) :: reduce,factor,this_wk
      REAL(DP), ALLOCATABLE :: vec_drhoz(:),veclc(:,:),rdrhoz(:,:)
      TYPE(json_file) :: json
      INTEGER :: iunit
      TYPE(bar_type) :: barra
      !
      ...
      DO iks = 1,kpt_pool%nloc
         ...
         DO ia = 1,nat
            ...
            CALL wbse_get_dvpsi_gamma_nonlocal(ia,aux1,dvpsi)
            !
            ! 2) vec_drhoz_i = < z_vector | dvpsi_i >
            !
            DO ipol = 1,3
               !
               reduce = 0._DP
               ...
               IF(gstart == 2) THEN
                  ...
                  DO lbnd = 1,nbnd_do
                     reduce = reduce &
                     & - REAL(zvector(1,lbnd,iks),KIND=DP)*REAL(dvpsi(1,lbnd,ipol),KIND=DP)
                  ENDDO
                 !$acc end parallel
               ENDIF
               !
               vec_drhoz(3*ia-3+ipol) = vec_drhoz(3*ia-3+ipol) + this_wk*reduce
               !
            ENDDO
            !
      ...
      ENDDO
      !
      CALL mp_sum(vec_drhoz,intra_bgrp_comm)
      CALL mp_sum(vec_drhoz,inter_bgrp_comm)
      CALL mp_sum(vec_drhoz,inter_pool_comm)
      !
      ...
      ENDIF
      !
      CALL force_lc(nat,tau,ityp,ntyp,alat,omega,ngm,ngl,igtongl,g,rdrhoz(:,1),gstart,gamma_only,&
      & vloc,veclc)
      !
      veclc(:,:) = -factor*veclc
      !
      DO ia = 1,nat
         DO ipol = 1,3
            vec_drhoz(3*ia-3+ipol) = vec_drhoz(3*ia-3+ipol) + veclc(ipol,ia)
         ENDDO
      ENDDO
      !
      outvec(:) = outvec+vec_drhoz
      !
      IF(.NOT. l_nac) THEN
         !
         ...
         DO ia = 1,nat
            ...
            ! forces = - gradients
            !
            WRITE(stdout,FMT_FORCE) ia,ityp(ia),(-vec_drhoz(3*ia-3+ipol),ipol = 1,3)
            !
         ENDDO
         !
      ELSEIF(l_genac .AND. .NOT. do_eenac) THEN
         !
         CALL io_push_title('geNAC drhoz')
         !
         DO ia = 1,nat
            !
            WRITE(stdout,FMT_GENAC) ia,ityp(ia),(vec_drhoz(3*ia-3+ipol),ipol = 1,3)
            !
         ENDDO
         !
      ELSEIF(l_eenac .AND. do_eenac) THEN
         ...
         DO ia = 1,nat
            !
            WRITE(stdout,FMT_EENAC) ia,ityp(ia),(vec_drhoz(3*ia-3+ipol),ipol = 1,3)
            !
         ENDDO
         !
      ENDIF
      ...
      IF(mpime == root) THEN
         ...
         CALL json%load(filename=TRIM(logfile))
         IF(.NOT. l_nac) THEN
            CALL json%add('output.forces.forces_drhoz',-vec_drhoz(1:n))
         ELSEIF(l_genac .AND. .NOT. do_eenac) THEN
            CALL json%add('output.nac_vec.genac_drhoz',vec_drhoz(1:n))
         ELSEIF(l_eenac .AND. do_eenac) THEN
            CALL json%add('output.nac_vec.eenac_drhoz',vec_drhoz(1:n))
         ENDIF
         !
         OPEN(NEWUNIT=iunit,FILE=TRIM(logfile))
         CALL json%print(iunit)
    ...
    SUBROUTINE wbse_get_dvpsi_gamma_nonlocal(i_at,dvg_tmp,dvpsi)
      ...
      DO ic = 1,3
         ...
         CALL DGEMM('C','N',nh_nt,band_group%nloc,2*npw,2._DP,work,2*npwx,dvg_tmp,2*npwx,0._DP,&
         & bec1,nh_nt)
         !$acc end host_data
         !
         IF(gstart == 2) THEN
         ...
         CALL DGEMM('N','N',2*npw,band_group%nloc,nh_nt,1._DP,vkb(1,jkb+1),2*npwx,bec1,nh_nt,1._DP,&
         & dvpsi(1,1,ic),2*npwx)
         !$acc end host_data
         !
         ...
         CALL DGEMM('C','N',nh_nt,band_group%nloc,2*npw,2._DP,vkb(1,jkb+1),2*npwx,dvg_tmp,2*npwx,&
         & 0._DP,bec2,nh_nt)
         !$acc end host_data
         !
         IF(gstart == 2) THEN
