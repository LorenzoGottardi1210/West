!
! Copyright (C) 2015-2026 M. Govoni
! This file is distributed under the terms of the
! GNU General Public License. See the file `License'
! in the root directory of the present distribution,
! or http://www.gnu.org/copyleft/gpl.txt .
!
! This file is part of WEST.
!
! Contributors to this file:
! Ngoc Linh Nguyen, Victor Yu
!
!----------------------------------------------------------------------------
SUBROUTINE wbse_localization(current_spin,nbnd_s,nbnd_e,evc_loc,ovl_mat,l_restart)
  !----------------------------------------------------------------------------
  !
  USE kinds,                ONLY : DP
  USE westcom,              ONLY : localization,wann_ng
  USE pwcom,                ONLY : npw,npwx
  USE gvect,                ONLY : gstart
  USE mp_global,            ONLY : intra_bgrp_comm
  USE mp,                   ONLY : mp_sum
  USE qbox_interface,       ONLY : load_qbox_wfc
  USE check_ovl_wfc,        ONLY : read_bisection_loc,check_ovl_bisection
  USE wbse_io,              ONLY : write_umatrix_and_omatrix,read_umatrix_and_omatrix
  USE distribution_center,  ONLY : band_group
  USE class_idistribute,    ONLY : idistribute
  USE wavefunctions,        ONLY : evc
#if defined(__CUDA)
  USE cublas
#endif
  !
  IMPLICIT NONE
  !
  INTEGER,INTENT(IN) :: current_spin,nbnd_s,nbnd_e
  REAL(DP),INTENT(OUT) :: ovl_mat(nbnd_e-nbnd_s+1,nbnd_e-nbnd_s+1)
  COMPLEX(DP),INTENT(OUT) :: evc_loc(npwx,nbnd_e-nbnd_s+1)
  LOGICAL,INTENT(IN) :: l_restart
  !
  LOGICAL :: l_wann
  INTEGER :: ibnd,jbnd,ibnd_g,jbnd_g
  INTEGER :: bisec_i,bisec_j
  INTEGER :: nbnd_do
  INTEGER,ALLOCATABLE :: bisec_loc(:)
  REAL(DP) :: ovl_val
  REAL(DP),ALLOCATABLE :: a_real(:,:,:)
  REAL(DP),ALLOCATABLE :: u_real(:,:)
  COMPLEX(DP),ALLOCATABLE :: u_cmplx(:,:)
  COMPLEX(DP),ALLOCATABLE :: evc_tmp(:,:)
#if !defined(__CUDA)
  REAL(DP),EXTERNAL :: DDOT
#endif
  !
  CALL start_clock('local')
  !
  SELECT CASE(localization)
  CASE('W','w')
     l_wann = .TRUE.
  CASE('B','b')
     l_wann = .FALSE.
  END SELECT
  !
  nbnd_do = nbnd_e-nbnd_s+1
  !
  IF(.NOT. l_restart) THEN
     !
     ovl_mat(:,:) = 0._DP
     !
     ALLOCATE(u_cmplx(nbnd_do,nbnd_do))
     !
     IF(l_wann) THEN
        !
        band_group = idistribute()
        CALL band_group%init(nbnd_do,'i','wann_local',.TRUE.)
        !
        ALLOCATE(a_real(nbnd_do,nbnd_do,2*wann_ng))
        ALLOCATE(u_real(nbnd_do,nbnd_do))
        !
        ! compute unitary rotation matrix
        !
        CALL wbse_wann_local(nbnd_s,nbnd_e,a_real,u_real)
        !
        u_cmplx(:,:) = CMPLX(u_real,KIND=DP)
        !
        DEALLOCATE(a_real)
        DEALLOCATE(u_real)
        !
        ! compute localized wfc
        !
        !$acc enter data copyin(u_cmplx)
        !
        !$acc host_data use_device(evc,u_cmplx,evc_loc)
        CALL ZGEMM('N','N',npw,nbnd_do,nbnd_do,(1._DP,0._DP),evc(1,nbnd_s),npwx,u_cmplx,nbnd_do,&
        & (0._DP,0._DP),evc_loc,npwx)
        !$acc end host_data
        !
        !$acc exit data delete(u_cmplx)
        !
        ! compute overlap
        !
        CALL wbse_wann_ovl(nbnd_s,nbnd_e,evc_loc,ovl_mat)
        !
     ELSE
        !
        ! load localized wfc
        !
        ALLOCATE(evc_tmp(npwx,nbnd_e))
        !
        CALL load_qbox_wfc(current_spin,nbnd_e,evc_tmp)
        !
        evc_loc(:,:) = evc_tmp(:,nbnd_s:nbnd_e)
        !
        DEALLOCATE(evc_tmp)
        !
        ! compute unitary rotation matrix
        !
        u_cmplx(:,:) = (0._DP,0._DP)
        !
        DO ibnd = 1,nbnd_do
           !
           ibnd_g = ibnd+nbnd_s-1
           !
           DO jbnd = 1,nbnd_do
              !
#if !defined(__CUDA)
              u_cmplx(ibnd,jbnd) = 2._DP*DDOT(2*npwx,evc(:,ibnd_g),1,evc_loc(:,jbnd),1)
#endif
              !
              IF(gstart == 2) THEN
                 u_cmplx(ibnd,jbnd) = u_cmplx(ibnd,jbnd) &
                 & - CMPLX(REAL(evc(1,ibnd_g),KIND=DP)*REAL(evc_loc(1,jbnd),KIND=DP),KIND=DP)
              ENDIF
              !
           ENDDO
        ENDDO
        !
        CALL mp_sum(u_cmplx,intra_bgrp_comm)
        !
        ALLOCATE(bisec_loc(nbnd_e))
        !
        ! read bisection localization from file
        !
        CALL read_bisection_loc(current_spin,nbnd_e,bisec_loc)
        !
        ! compute overlap
        !
        DO ibnd = 1,nbnd_do
           ibnd_g = ibnd+nbnd_s-1
           !
           DO jbnd = 1,nbnd_do
              !
              jbnd_g = jbnd+nbnd_s-1
              !
              bisec_i = bisec_loc(ibnd_g)
              bisec_j = bisec_loc(jbnd_g)
              !
              CALL check_ovl_bisection(bisec_i,bisec_j,ovl_val)
              !
              ovl_mat(ibnd,jbnd) = ovl_val
              !
           ENDDO
        ENDDO
        !
        DEALLOCATE (bisec_loc)
        !
     ENDIF
     !
     ! save overlap matrix and unitary transformation matrix
     !
     CALL write_umatrix_and_omatrix(nbnd_do,current_spin,u_cmplx,ovl_mat)
     !
     DEALLOCATE(u_cmplx)
     !
  ELSE
     !
     ALLOCATE(u_cmplx(nbnd_do,nbnd_do))
     !
     ! read overlap matrix and unitary transformation matrix
     !
     CALL read_umatrix_and_omatrix(nbnd_do,current_spin,u_cmplx,ovl_mat)
     !
     !$acc enter data copyin(u_cmplx)
     !
     !$acc host_data use_device(evc,u_cmplx,evc_loc)
     CALL ZGEMM('N','N',npw,nbnd_do,nbnd_do,(1._DP,0._DP),evc(1,nbnd_s),npwx,u_cmplx,nbnd_do,&
     & (0._DP,0._DP),evc_loc,npwx)
     !$acc end host_data
     !
     !$acc exit data delete(u_cmplx)
     DEALLOCATE(u_cmplx)
     !
  ENDIF
  !
  CALL stop_clock('local')
  !
END SUBROUTINE
!
!----------------------------------------------------------------------------
SUBROUTINE wbse_wann_local(nbnd_s,nbnd_e,amat,umat)
  !----------------------------------------------------------------------------
  !
  USE kinds,                ONLY : DP
  USE westcom,              ONLY : wann_ng
  USE fft_base,             ONLY : dffts
  USE fft_at_gamma,         ONLY : double_invfft_gamma,single_invfft_gamma
  USE pwcom,                ONLY : npw,npwx
  USE mp_global,            ONLY : inter_image_comm,intra_bgrp_comm
  USE mp,                   ONLY : mp_sum
  USE wann_loc_wfc,         ONLY : wann_calc_proj,wann_jade
  USE distribution_center,  ONLY : band_group
  USE io_push,              ONLY : io_push_title
  USE bar,                  ONLY : bar_type,start_bar_type,update_bar_type,stop_bar_type
  USE wavefunctions,        ONLY : evc,psic
#if defined(__CUDA)
  USE cublas
#endif
  !
  IMPLICIT NONE
  !
  INTEGER,INTENT(IN) :: nbnd_s,nbnd_e
  REAL(DP),INTENT(OUT) :: amat(nbnd_e-nbnd_s+1,nbnd_e-nbnd_s+1,2*wann_ng)
  REAL(DP),INTENT(OUT) :: umat(nbnd_e-nbnd_s+1,nbnd_e-nbnd_s+1)
  !
  INTEGER :: ibnd,jbnd,ibnd_l,ibnd_g,jbnd_g,ir,il
  INTEGER :: dffts_nnr
  INTEGER :: nbnd_do
  INTEGER :: barra_load
  REAL(DP) :: reduce
  REAL(DP) :: val(8)
  REAL(DP),ALLOCATABLE :: proj(:,:)
  REAL(DP),ALLOCATABLE :: aux(:)
  TYPE(bar_type) :: barra
  !
  nbnd_do = nbnd_e-nbnd_s+1
  dffts_nnr = dffts%nnr
  !
  ! compute unitary rotation matrix
  !
  ALLOCATE(proj(dffts_nnr,2*wann_ng))
  ALLOCATE(aux(dffts_nnr))
  !$acc enter data create(aux)
  !
  CALL wann_calc_proj(proj)
  !
  !$acc enter data copyin(proj)
  !
  amat(:,:,:) = 0._DP
  !
  barra_load = 0
  DO ibnd_l = 1,band_group%nloc
     ibnd = band_group%l2g(ibnd_l)
     DO jbnd = ibnd,nbnd_do
        barra_load = barra_load+1
     ENDDO
  ENDDO
  !
  CALL io_push_title('Wannier (A matrices)')
  !
  CALL start_bar_type(barra,'wann',barra_load)
  !
  DO ibnd_l = 1,band_group%nloc
     !
     ibnd = band_group%l2g(ibnd_l)
     ibnd_g = ibnd+nbnd_s-1
     !
     CALL single_invfft_gamma(dffts,npw,npwx,evc(:,ibnd_g),psic,'Wave')
     !
     !$acc kernels present(aux,psic)
     aux(:) = REAL(psic,KIND=DP)
     !$acc end kernels
     !
     DO jbnd = ibnd,nbnd_do,2
        !
        jbnd_g = jbnd+nbnd_s-1
        !
        IF(jbnd < nbnd_do) THEN
           !
           CALL double_invfft_gamma(dffts,npw,npwx,evc(:,jbnd_g),evc(:,jbnd_g+1),psic,'Wave')
           !
           DO il = 1,2*wann_ng
              !
              reduce = 0._DP
              !
              !$acc parallel loop reduction(+:reduce) present(aux,psic,proj) copy(reduce)
              DO ir = 1,dffts_nnr
                 reduce = reduce+aux(ir)*REAL(psic(ir),KIND=DP)*proj(ir,il)
              ENDDO
              !$acc end parallel
              !
              val(il) = reduce
              !
           ENDDO
           !
           amat(ibnd,jbnd,1:2*wann_ng) = val(1:2*wann_ng)
           IF(ibnd /= jbnd) amat(jbnd,ibnd,1:2*wann_ng) = val(1:2*wann_ng)
           !
           DO il = 1,2*wann_ng
              !
              reduce = 0._DP
              !
              !$acc parallel loop reduction(+:reduce) present(aux,psic,proj) copy(reduce)
              DO ir = 1,dffts_nnr
                 reduce = reduce+aux(ir)*AIMAG(psic(ir))*proj(ir,il)
              ENDDO
              !$acc end parallel
              !
              val(il) = reduce
              !
           ENDDO
           !
           amat(ibnd,jbnd+1,1:2*wann_ng) = val(1:2*wann_ng)
           IF(ibnd /= jbnd+1) amat(jbnd+1,ibnd,1:2*wann_ng) = val(1:2*wann_ng)
           !
        ELSE
           !
           CALL single_invfft_gamma(dffts,npw,npwx,evc(:,jbnd_g),psic,'Wave')
           !
           DO il = 1,2*wann_ng
              !
              reduce = 0._DP
              !
              !$acc parallel loop reduction(+:reduce) present(aux,psic,proj) copy(reduce)
              DO ir = 1,dffts_nnr
                 reduce = reduce+aux(ir)*REAL(psic(ir),KIND=DP)*proj(ir,il)
              ENDDO
              !$acc end parallel
              !
              val(il) = reduce
              !
           ENDDO
           !
           amat(ibnd,jbnd,1:2*wann_ng) = val(1:2*wann_ng)
           IF(ibnd /= jbnd) amat(jbnd,ibnd,1:2*wann_ng) = val(1:2*wann_ng)
           !
        ENDIF
        !
     ENDDO
     !
     CALL update_bar_type(barra,'wann',nbnd_do-ibnd+1)
     !
  ENDDO
  !
  CALL mp_sum(amat,intra_bgrp_comm)
  CALL mp_sum(amat,inter_image_comm)
  !
  CALL stop_bar_type(barra,'wann')
  !
  CALL wann_jade(nbnd_do,amat,2*wann_ng,umat)
  !
  !$acc exit data delete(proj,aux)
  DEALLOCATE(proj)
  DEALLOCATE(aux)
  !
END SUBROUTINE
!
!----------------------------------------------------------------------------
SUBROUTINE wbse_wann_ovl(nbnd_s,nbnd_e,evc_loc,omat)
  !----------------------------------------------------------------------------
  !
  USE kinds,                ONLY : DP
  USE fft_base,             ONLY : dffts
  USE fft_at_gamma,         ONLY : double_invfft_gamma,single_invfft_gamma
  USE pwcom,                ONLY : npw,npwx
  USE mp_global,            ONLY : inter_image_comm
  USE mp,                   ONLY : mp_sum
  USE check_ovl_wfc,        ONLY : check_ovl_wannier
  USE distribution_center,  ONLY : band_group
  USE io_push,              ONLY : io_push_title
  USE bar,                  ONLY : bar_type,start_bar_type,update_bar_type,stop_bar_type
  USE wavefunctions,        ONLY : psic
#if defined(__CUDA)
  USE cublas
#endif
  !
  IMPLICIT NONE
  !
  INTEGER,INTENT(IN) :: nbnd_s,nbnd_e
  COMPLEX(DP),INTENT(IN) :: evc_loc(npwx,nbnd_e-nbnd_s+1)
  REAL(DP),INTENT(OUT) :: omat(nbnd_e-nbnd_s+1,nbnd_e-nbnd_s+1)
  !
  INTEGER :: ibnd,jbnd,ibnd_l
  INTEGER :: dffts_nnr
  INTEGER :: nbnd_do
  INTEGER :: barra_load
  REAL(DP) :: ovl_val
  REAL(DP),ALLOCATABLE :: aux(:)
  REAL(DP),ALLOCATABLE :: aux2(:)
  TYPE(bar_type) :: barra
  !
  nbnd_do = nbnd_e-nbnd_s+1
  dffts_nnr = dffts%nnr
  !
  ALLOCATE(aux(dffts_nnr))
  ALLOCATE(aux2(dffts_nnr))
  !$acc enter data create(aux,aux2)
  !
  omat(:,:) = 0._DP
  !
  CALL io_push_title('Wannier (overlap)')
  !
  CALL start_bar_type(barra,'wann',barra_load)
  !
  DO ibnd_l = 1,band_group%nloc
     !
     ibnd = band_group%l2g(ibnd_l)
     !
     CALL single_invfft_gamma(dffts,npw,npwx,evc_loc(:,ibnd),psic,'Wave')
     !
     !$acc kernels present(aux,psic)
     aux(:) = REAL(psic,KIND=DP)
     !$acc end kernels
     !
     DO jbnd = ibnd,nbnd_do,2
        !
        IF(jbnd < nbnd_do) THEN
           !
           CALL double_invfft_gamma(dffts,npw,npwx,evc_loc(:,jbnd),evc_loc(:,jbnd+1),psic,&
           & 'Wave')
           !
           !$acc kernels present(aux2,psic)
           aux2(:) = REAL(psic,KIND=DP)
           !$acc end kernels
           !
           CALL check_ovl_wannier(aux,aux2,ovl_val)
           !
           omat(ibnd,jbnd) = ovl_val
           omat(jbnd,ibnd) = ovl_val
           !
           !$acc kernels present(aux2,psic)
           aux2(:) = AIMAG(psic)
           !$acc end kernels
           !
           CALL check_ovl_wannier(aux,aux2,ovl_val)
           !
           omat(ibnd,jbnd+1) = ovl_val
           omat(jbnd+1,ibnd) = ovl_val
           !
        ELSE
           !
           CALL single_invfft_gamma(dffts,npw,npwx,evc_loc(:,jbnd),psic,'Wave')
           !
           !$acc kernels present(aux2,psic)
           aux2(:) = REAL(psic,KIND=DP)
           !$acc end kernels
           !
           CALL check_ovl_wannier(aux,aux2,ovl_val)
           !
           omat(ibnd,jbnd) = ovl_val
           omat(jbnd,ibnd) = ovl_val
           !
        ENDIF
        !
     ENDDO
     !
     CALL update_bar_type(barra,'wann',nbnd_do-ibnd+1)
     !
  ENDDO
  !
  CALL mp_sum(omat,inter_image_comm)
  !
  CALL stop_bar_type(barra,'wann')
  !
  !$acc exit data delete(aux,aux2)
  DEALLOCATE(aux)
  DEALLOCATE(aux2)
  !
END SUBROUTINE
